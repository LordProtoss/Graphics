...
public:
	...
	void DrawTexture(GLuint texture_id, GLuint vertexbuffer, GLuint uvbuffer) {
		glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer);
		glBufferData(GL_ARRAY_BUFFER, vertices_.size() * sizeof(glm::vec3), &vertices_[0], GL_STATIC_DRAW);
	...
	virtual void Draw(GLuint texture_id, GLuint vertexbuffer, GLuint uvbuffer, GLuint normalbuffer, bool shader=true) {
		if (shader) {
			DrawShader(texture_id, vertexbuffer, uvbuffer, normalbuffer);
		}
		else {
			DrawTexture(texture_id, vertexbuffer, uvbuffer);
		}
	}
...
public:
	explicit Floor(const glm::vec3& position)
		: Object(position, glm::vec3(0.0f, 1.0f, 0.0f), 1000.0f, 0.0f, "floor.obj", "floor.DDS") {}
	...
	virtual Object* Act(const std::vector<Object*>& objects);
};
...
public:
	...
	void Save(std::iostream& file) override {
		Object::Save(file);
		file << damage_ << " " << end_time_ << std::endl;
	}

	void Load(std::iostream& file) override {
		Object::Load(file);
		file >> damage_ >> end_time_;
	}
	...
	do {
		...
		for (Object* obj : objects) {
			if (dynamic_cast<Projectile*>(obj) != nullptr) {
				...
				obj->Draw(SimpleTextureID, vertexbuffer, uvbuffer, normalbuffer, false);
			}
		}
